...
export const verifyEmail = async (req, res, next) => {
  ...
  try {
    if (!userId || !verificationToken) {
      return next(createHttpError(400, "UserId or VerificationToken not provided"));
    }
    // Find user
    ...
    if (user.verificationTokenExpires < Date.now()) {
      ...
      await user.save();
      return next(createHttpError(410, "Verification token has expired, please request a new one"));
    } else {
...
export const resetPassword = async (req, res, next) => {
  ...
  try {
    if (!newPassword || !confirmPassword) {
      return next(createHttpError(400, "New password and confirm password are required"));
    }
    // Find user by userId and passwordToken
    const user = await User.findOne({ _id: userId, passwordResetToken: passwordToken })
    ...
    if (user.passwordResetTokenExpires < Date.now()) {
      ...
      await user.save();
      return next(createHttpError(410, "Password reset token has expired, please request a new one"));
    }
